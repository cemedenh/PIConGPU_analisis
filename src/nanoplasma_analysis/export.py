...
from .core import NanoPlasmaRun as _NanoPlasmaRunBase, extract_step_from_filename
...
class NanoPlasmaRun(_NanoPlasmaRunBase):
    ...
    def export_reduced_h5(
        ...
        *,
        ...
        n_E: int = 800,
        # Energy binning
        energy_binning: str = "linear",  # "linear" | "log" | "piecewise" | "custom"
        # Backward-compatibility: if not None, overrides energy_binning (True->log, False->linear)
        log_energy: bool | None = None,
        # Optional: custom energy bin edges in eV (length NE+1, strictly increasing)
        E_edges_eV: np.ndarray | None = None,
        # Optional: piecewise segments [(start_eV, stop_eV, step_eV), ...]
        E_piecewise: list[tuple[float, float, float]] | None = None,
        ...
        if log_energy is not None:
            # legacy switch
            energy_binning = "log" if log_energy else "linear"

        if E_edges_eV is not None:
            E_edges = np.asarray(E_edges_eV, dtype=float)
            if E_edges.ndim != 1 or E_edges.size < 2:
                raise ValueError("E_edges_eV must be a 1D array with length >= 2")
            if not np.all(np.isfinite(E_edges)):
                raise ValueError("E_edges_eV contains non-finite values")
            if not np.all(np.diff(E_edges) > 0):
                raise ValueError("E_edges_eV must be strictly increasing")
            # clip/ensure lower bound
            if E_edges[0] < 0:
                raise ValueError("E_edges_eV must be >= 0 eV")
        else:
            mode = str(energy_binning).strip().lower()
            if mode in ("linear", "lin"):
                E_edges = np.linspace(E_min_eV, E_max_eV, n_E + 1)
            elif mode in ("log", "logspace", "geom"):
                E_edges = _log_edges(E_min_eV, E_max_eV, n_E)
            elif mode in ("piecewise", "pw"):
                if E_piecewise is None:
                    # default piecewise grid: very fine near 0, coarser at high E
                    E_piecewise = [
                        (0.0, 2.0, 0.002),
                        (2.0, 50.0, 0.05),
                        (50.0, E_max_eV, 0.5),
                ...
                edges = []
                for a, b, d in E_piecewise:
                    if d <= 0:
                        raise ValueError("E_piecewise step must be > 0")
                    if b <= a:
                        raise ValueError("E_piecewise stop must be > start")
                    # include endpoint with a tiny epsilon
                    edges.append(np.arange(a, b + 0.5 * d + 1e-12, d, dtype=float))
                E_edges = np.unique(np.concatenate(edges))
                E_edges = E_edges[(E_edges >= 0) & (E_edges <= E_max_eV + 1e-9)]
                if E_edges.size < 2:
                    raise RuntimeError("Piecewise E_edges construction failed")
                # ensure start at >=E_min_eV
                if E_edges[0] < E_min_eV:
                    E_edges[0] = E_min_eV
            else:
                raise ValueError(f"Unknown energy_binning='{energy_binning}' (use linear|log|piecewise or pass E_edges_eV)")
        ...
        with h5py.File(out_h5, "w") as h5:
            ...
            gmeta.attrs["source"] = "nanoplasma_analysis.export.NanoPlasmaRun.export_reduced_h5"
